...
export function parseAeronStat(stats: string): AeronStatParsed {
  ...
  const heartbeatAgeMs = firstLine
    .split(',')[2]
    .split(' ')[3]
    .replace('ms', '');
...
function parseSubscriptions(lines: string[]): AeronStatSubscription[] {
  ...
  return aeronStatSubscriptions
    .sort((left, right) => (left.streamId > right.streamId ? -1 : 1))
    .reverse();
}
...
function parseSubscription(
  ...
  if (rightPart2[2] !== undefined) {
    if (defaultStreams.has(rightPart2[2].trim())) {
      streamIdDecorated =
        rightPart2[2].trim() + ' - ' + defaultStreams.get(rightPart2[2].trim());
    }
  }
  ...
  for (let i = 2; i < lines.length - 1; i++) {
    ...
    if (rightSide.startsWith('Cluster Errors')) {
      ...
      consensusModuleState =
        consensusModuleStateValues.get(numericLeftSide) === undefined
          ? '?'
          : consensusModuleStateValues.get(numericLeftSide)!;
    } else if (rightSide.startsWith('Cluster node role')) {
      nodeRole =
        clusterNodeRoleValues.get(numericLeftSide) === undefined
          ? '?'
          : clusterNodeRoleValues.get(numericLeftSide)!;
      likelyClusterStat = true;
    } else if (rightSide.startsWith('Cluster election state')) {
      electionState =
        clusterElectionStateValues.get(numericLeftSide) === undefined
          ? '?'
          : clusterElectionStateValues.get(numericLeftSide)!;
...
function parseTopLevelAeronStats(lines: string[]): TopLevelAeronStats {
  ...
  for (let i = 2; i < lines.length - 1; i++) {
    ...
    if (rightSide.startsWith('Bytes sent')) {
      ...
      rightSide.startsWith('Conductor max cycle time doing its work')
    ) {
      conductorMaxCycleTime = parseInt(numericLeftSide);
      conductorMaxCycleTimeSeconds = conductorMaxCycleTime / 1000000000;
    } else if (
      rightSide.startsWith('Conductor work cycle exceeded threshold')
    ) {
...
function parseSendSockets(lines: string[]): SendSocket[] {
  ...
  const linesWithChannels = lines.filter(
    (line) => line.indexOf('snd-channel') > -1
  );
  ...
  return sendSockets
    .sort((post1, post2) => (post1.hostAndPort > post2.hostAndPort ? -1 : 1))
    .reverse();
}
...
function parseReceiveSockets(lines: string[]): ReceiveSocket[] {
  ...
  const linesWithChannels = lines.filter(
    (line) => line.indexOf('rcv-channel') > -1
  );
  ...
  return receiveSockets
    .sort((post1, post2) => (post1.hostAndPort > post2.hostAndPort ? -1 : 1))
    .reverse();
}

function parseAeronStatFeaturesDetected(
  lines: string[]
): AeronStatFeatureDetected[] {
  ...
  for (let i = 2; i < lines.length - 1; i++) {
    if (lines[i].indexOf('Archive Control Sessions') > -1) {
      aeronStatFeaturesDetected.add(
        AeronStatFeatureDetected.AERON_ARCHIVE_SERVER
      );
    ...
    for (let j = 0; j < subscriptions.length; j++) {
      ...
      if (subscription.sessionId === publication.sessionId) {
        internalFlows.push({
          ...
          diff: (
            subscription.subPositionNumeric -
            publication.pubPositionSampledNumeric
          ).toLocaleString()
        });
      }
    }
