...
export function recommend(aeronStatParsed : AeronStatParsed) : AeronStatOutput {
    ...
    const recommendations = checkStats(aeronStatParsed.internalFlows, aeronStatParsed.clusterData, aeronStatParsed.topLevelAeronStats);
...
function checkStats(internalFlows: AeronStatInternalFlow[], clusterData?: AeronClusterDetails, topLevelAeronStats?: TopLevelAeronStats) : AeronStatRecommendation[] {
    ...
    if (topLevelAeronStats.errors > 0) {
        recs.push({level: 'WARN', message : 'Aeron errors were raised. Review raised errors from logs or with Aeron ErrorStat Tool', weight:1000});
    } 

    for(let i = 0; i < internalFlows.length; i++) {
        const logDiff = parseInt(stripComma(internalFlows[i].diff));
        if (Math.abs(logDiff) > 1000) {
            recs.push({level: 'WARN', message : 'Stream ' + internalFlows[i].streamId + ' is running behind by '+Math.abs(logDiff).toLocaleString()+'. Production is faster than consumption, and may result in backpressure.', weight:100});
        }
    }
    ...
    if (topLevelAeronStats.conductorWorkCycleExceededCount > 0) {
        ...
        recs.push({level: 'WARN', message : 'The conductor work cycle exceeded the limit '+topLevelAeronStats.conductorWorkCycleExceededCount.toLocaleString()+ ' ' + times + ' with a max of ' + secs + ' seconds. Confirm there are enough system resources on the host.', weight:800});
    }

    if (topLevelAeronStats.shortSends > 0) {
        recs.push({level: 'WARN', message : 'Short sends detected. Investigate the SO_SNDBUF size & network equipment if this value is high & increasing.', weight:500});
    } 

    if (topLevelAeronStats.naksSent > 0) {
        recs.push({level: 'WARN', message : 'The Media Driver had to send ' + topLevelAeronStats.naksSent.toLocaleString() + ' NAKs in order to request missing packet(s).', weight:500});
    }

    if (topLevelAeronStats.naksReceived > 0) {
        recs.push({level: 'WARN', message : 'The Media Driver received '+ topLevelAeronStats.naksReceived.toLocaleString() +' NAKs in order to recover missing packet(s).', weight:500});
    }

    if (topLevelAeronStats.unblockedPublications > 0) {
        recs.push({level: 'WARN', message : 'The Media Driver had to unblock ' + topLevelAeronStats.unblockedPublications.toLocaleString() + ' publications in order to send data to subscribers. Confirm that usages of tryClaim are committing within timeout limits.', weight:500});
    }
    ...
    if (parseInt(clusterData.clusterErrors) > 0) {
        recs.push({level: 'WARN', message : 'Cluster errors were raised. Review raised errors from logs or with Aeron Cluster Tool', weight:1001});
    } 

    if (parseInt(clusterData.clusterContainerErrors) > 0) {
        recs.push({level: 'WARN', message : 'Cluster container errors were raised. Review raised errors from logs or with Aeron Cluster Tool', weight:1002});
    } 
    ...
    if (logDiff !== 0) {
        recs.push({level: 'WARN', message : 'Cluster container is running behind the cluster commit position by ' + Math.abs(logDiff).toLocaleString()+ ' bytes.', weight:900});
    }
